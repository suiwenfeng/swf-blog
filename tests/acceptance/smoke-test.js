...
module('Acceptance | smoke test', function(hooks) {
  ...
  test('navigate to all routes', async function(assert) {
    ...
    assert.dom('head meta[property="og:title"]', document).hasAttribute('content', 'swf-blog: The simple JAM stack blog');

    await click('.post-card-content-link');
    assert.equal(currentURL(), '/welcome');
    assert.dom('head meta[property="og:title"]', document).hasAttribute('content', 'Welcome to swf-blog');

...
module('Acceptance | meta test', function(hooks) {
  ...
  test('index meta', async function(assert) {
    ...
    assert.dom('head meta[property="og:title"]', document)
      .hasAttribute('content', 'swf-blog: The simple JAM stack blog');
    ...
    assert.dom('head link[rel="canonical"]', document)
      .hasAttribute('href', 'https://swf-blog.netlify.com/');
    ...
    assert.dom('head meta[property="og:image"]', document)
      .hasAttribute('content', 'https://swf-blog.netlify.com/images/logo.png');
  ...
  test('content meta', async function(assert) {
    ...
    assert.dom('head meta[property="og:title"]', document)
      .hasAttribute('content', 'Welcome to swf-blog');
    assert.dom('head meta[name="description"]', document)
      .hasAttribute('content', `Hey! Welcome to swf-blog, it's great to have you :) We know that first impressions are important, so we've populated your new site with some initial Getting Started posts that will...`);
    ...
    assert.dom('head link[rel="canonical"]', document)
      .hasAttribute('href', 'https://swf-blog.netlify.com/welcome/');
    ...
    assert.dom('head meta[property="og:image"]', document)
      .hasAttribute('content', 'https://swf-blog.netlify.com/images/welcome.jpg');
    ...
    assert.dom('head meta[name="description"]', document)
      .hasAttribute('content', `swf-blog does not, and will never have the concept of user roles! This is because it is entirely a static system. You can assign authors to posts and generate new ones using...`);
    assert.dom('head meta[name="twitter:label1"]', document)
      .hasAttribute('content', 'Written by');
    assert.dom('head meta[name="twitter:data1"]', document)
    ...
    assert.dom('head meta[name="twitter:data2"]', document)
      .hasAttribute('content', 'Getting Started, Recent posts');
    assert.dom('head link[rel="canonical"]', document)
      .hasAttribute('href', 'https://swf-blog.netlify.com/managing-users/');
    // I'm not testing the value of conent here to avoid timezone variations
    assert.dom('head meta[property="article:published_time"]', document)
      .hasAttribute('content');
    assert.dom('head meta[property="article:tag"]', document)
    ...
    assert.dom('head meta[property="og:type"]', document)
      .hasAttribute('content', 'article');
    assert.dom('head meta[property="og:image"]', document)
      .hasAttribute('content', 'https://swf-blog.netlify.com/images/team.jpg');
  })
  ...
  test('page meta', async function(assert) {
    ...
    assert.dom('head meta[name="description"]', document)
      .hasAttribute('content', `Chris Manson is a Ember enthusiast and a member of the Ember Core Learning Team. swf-blog was an experimental project that came out of the work on converting the Ember Guides to...`);
    assert.dom('head meta[name="twitter:label1"]', document)
      .hasAttribute('content', 'Written by');
    assert.dom('head meta[name="twitter:data1"]', document)
      .hasAttribute('content', 'Chris Manson');
    assert.dom('head link[rel="canonical"]', document)
      .hasAttribute('href', 'https://swf-blog.netlify.com/page/chris-manson/');
    assert.dom('head meta[property="og:type"]', document)
      .hasAttribute('content', 'website');
    assert.dom('head meta[property="og:image"]', document)
      .hasAttribute('content', 'https://swf-blog.netlify.com/images/built-by.jpg');
  });
  ...
  test('author meta', async function(assert) {
    ...
    assert.dom('head link[rel="canonical"]', document)
      .hasAttribute('href', 'https://swf-blog.netlify.com/author/ghost/');
    assert.dom('head meta[property="og:type"]', document)
      .hasAttribute('content', 'website');
    assert.dom('head meta[property="og:image"]', document)
      .hasAttribute('content', 'https://swf-blog.netlify.com/images/ghost-icon.png');
  });
  ...
  test('tag meta', async function(assert) {
    ...
    assert.dom('head link[rel="canonical"]', document)
      .hasAttribute('href', 'https://swf-blog.netlify.com/tag/getting-started/');
    assert.dom('head meta[property="og:type"]', document)
      .hasAttribute('content', 'website');
    assert.dom('head meta[property="og:image"]', document)
      .hasAttribute('content', 'https://swf-blog.netlify.com/images/logo.png');
